# ------------------------------------------------------------
# Producers-Toolkit_v0.20.4d-dev.py
# ------------------------------------------------------------
# Version: v0.20.4d-dev
#
# Requirements:
#      Python 3.12+
#      PyQt6 (core)
...
class ProducersToolkit(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Producer's Toolkit v0.20.4d-dev")
    ...
    def open_preview_modal(self):
        ...
        try:
            styles = getSampleStyleSheet()
            styles.add(ParagraphStyle(name="Center12", alignment=1, fontSize=12, leading=15))
            formatted = [[Paragraph(str(cell), styles["Center12"]) for cell in row] for row in data]
            table = Table(formatted)
            ts = TableStyle([
                ("GRID", (0, 0), (-1, -1), 0.25, colors.grey),
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
