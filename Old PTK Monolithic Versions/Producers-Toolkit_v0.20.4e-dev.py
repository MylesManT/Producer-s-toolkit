# ------------------------------------------------------------
# Producers-Toolkit_v0.20.4e-dev.py
# ------------------------------------------------------------
# Version: v0.20.4e-dev
#
# Requirements:
#      Python 3.12+
#      PyQt6 (core)
...
from PyQt6.QtWidgets import (
    ...
    QFrame, QDialog, QTabWidget, QTextBrowser, QHeaderView
)
...
class ProducersToolkit(QMainWindow):
    """
    Main application window for Producer's Toolkit v0.20.4d-dev.
    """

    def __init__(self):
        super().__init__()
        self.setWindowTitle("Producer's Toolkit v0.20.4e-dev")
    ...
    def update_scene_row_for_box(self, box):
        ...
        shooting_time_item = QTableWidgetItem(str(timedelta(seconds=secs)))
        shooting_time_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)
        self.table.setItem(row, 6, shooting_time_item)
        self.trigger_recalc_with_row_fades()
    ...
    def populate_table(self):
        ...
        for i, sc in enumerate(self.scenes):
            ...
            heading_item.setTextAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
            
            page_len_item = QTableWidgetItem(page_len)
            page_len_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)
            
            mmss_item = QTableWidgetItem(mmss)
            mmss_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)
            
            shooting_time_item = QTableWidgetItem(str(timedelta(seconds=self.compute_scene_time(i))))
            shooting_time_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)

            self.table.setItem(i, 0, heading_item)
            self.table.setItem(i, 3, page_len_item)
            self.table.setItem(i, 4, mmss_item)
            self.table.setCellWidget(i, 5, setups_box)
            self.table.setItem(i, 6, shooting_time_item)
                ...
                for r, row in enumerate(data):
                    if r > 0:  # Skip header
                        if row[0].startswith("LUNCH"):
                            ...
                            ts.add("BACKGROUND", (0, r), (-1, r), colors.HexColor("#2196f3"))
                            ts.add("TEXTCOLOR", (0, r), (-1, r), colors.white)
                            ts.add("SPAN", (0, r), (-1, r))
                            ts.add("TOPPADDING", (0, r), (-1, r), 10)
                            ts.add("BOTTOMPADDING", (0, r), (-1, r), 10)
    ...
    def open_preview_modal(self):
        ...
        Falls back to CSV text preview if QtPDF not installed.
        Temporary preview files are deleted when the modal is closed.
        Auto-loads immediately on modal open.
        """
        # Create temporary preview files
        tempdir = tempfile.gettempdir()
        unique = str(uuid.uuid4())[:8]
        csv_path = os.path.join(tempdir, f"ptk_preview_{unique}.csv")
        pdf_path = os.path.join(tempdir, f"ptk_preview_{unique}.pdf")

        # Get actual table data for preview
        data = self.get_table_data()
        ...
        try:
            ...
            QMessageBox.critical(self, "Preview Error", f"Could not create CSV preview: {e}")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Preview Error", f"Could not create PDF preview: {e}")
            return
        ...
        def _cleanup():
            # Remove temporary preview files on close
